...
func main() {
	...
	log.Println(x)
	//sql, eee_ := GenerateInsertOrUpdateQueryNull(table["basename"], x, y, x)
	//
...
func readCSV0(file string) ([]string, [][]string, error) {
	f, err := os.Open(file)
	if err != nil {
		panic(err)
		return nil, nil, err
...
func GenerateInsertOrUpdateQueryNull(table string, columns []string, values [][]string, updateColumns []string) (string, error) {
	if len(columns) != len(values[0]) {
		panic("number of columns does not match number of values")
		return "", fmt.Errorf("number of columns does not match number of values")
	}
